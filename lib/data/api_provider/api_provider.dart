    ...
    try {
      ...
      if (isProgress) {
        if (formData['before'] != null && formData['after'] != null) {
          request.files.add(
              await http.MultipartFile.fromPath('before', formData['before']));
          request.files.add(
              await http.MultipartFile.fromPath('after', formData['after']));
        } else {
          throw Exception(
              "Missing 'before' or 'after' file for progress upload.");
        }
      } else {
        if (formData['image'] != null) {
          request.files.add(
              await http.MultipartFile.fromPath('image', formData['image']));
        } else {
      ...
      debugPrint(
          '====> API Response: [${response.statusCode}] $fullUrl\n${response.body}');
