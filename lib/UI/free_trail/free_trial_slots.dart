...
class FreeTrialSlots extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (_, a) {
        CustomToast.failToast(msg: "Please provide details");
      },
      child: Scaffold(
                                      ...
                                      return GestureDetector(
                                        onTap: () async {
                                          if (workOutController.freeTrialSlots
                                              .contains(slot.id.toString())) {
                                            workOutController.freeTrialSlots
                                                .remove(slot.id.toString());
                                            return;
                                          }
                                          if (workOutController
                                              ...
                                              2) {
                                            if (workOutController
                                                .freeTrialSlots.isEmpty) {
                                              workOutController.freeTrialSlots
                                                  .add(slot.id.toString());
                                            } else {
                                              bool contain = workOutController
                                                  .freeTrialSlots
                                                  .any((v) => time.slots.any(
                                                      (s) =>
                                                          s.id.toString() ==
                                                          v));
                                              if (contain) {
                                                CustomToast.failToast(
                                                    msg:
                                                        "You can select one slot from one day");
                                              } else {
                                                workOutController.freeTrialSlots
                                                    .add(slot.id.toString());
                                              }
                      ...
                      CustomButton(
                        ...
                        onPressed: () {
                          if (workOutController.freeTrialSlots.length < 2) {
                            CustomToast.failToast(
                                msg: "Please select at least two slot");
                            return;
                          }
  ...
  RxBool expandTile(TrainerSlot time) {
    bool contain = workOutController.freeTrialSlots
        .any((v) => time.slots.any((s) => s.id.toString() == v));
